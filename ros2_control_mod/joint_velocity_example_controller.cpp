...
// ═══════════════════════════════════════════════════════════════════════════
// update — 1000 Hz
// Reads velocity_commands_ (written by subscriber) and forwards to joints.
// Zero torques are sent until the first message arrives.
// ═══════════════════════════════════════════════════════════════════════════
controller_interface::return_type JointVelocityExampleController::update(
  ...
  for (int i = 0; i < num_joints; i++) {
    command_interfaces_[i].set_value(velocity_commands_[i]);
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// on_configure
// Same as original — reads robot description, sets collision behavior on
// real robot. Additionally creates the velocity command subscriber.
// ═══════════════════════════════════════════════════════════════════════════
CallbackReturn JointVelocityExampleController::on_configure(
  ...
  // ~/commands resolves to /joint_velocity_example_controller/commands
  // Accepts Float64MultiArray with 7 values in rad/s.
  // Values are clamped to ±kMaxVelocity before storing.
 

  RCLCPP_INFO(get_node()->get_logger(),
              "JointVelocityExampleController configured. "
              "Listening on ~/commands for velocity commands.");
  return CallbackReturn::SUCCESS;
}

// ═══════════════════════════════════════════════════════════════════════════
// on_activate
// Zero all commands on activation — robot holds position until first message.
// ═══════════════════════════════════════════════════════════════════════════
CallbackReturn JointVelocityExampleController::on_activate(
  ...
  RCLCPP_INFO(get_node()->get_logger(),
              "JointVelocityExampleController activated. "
              "Publish to ~/commands to move the robot.");
  velocity_sub_ =
              ...
              if (static_cast<int>(msg->data.size()) != num_joints) {
                RCLCPP_WARN_THROTTLE(get_node()->get_logger(),
                                    *get_node()->get_clock(), 1000,
                                    "Expected %d velocity commands, got %zu — ignoring",
...
// ═══════════════════════════════════════════════════════════════════════════
// on_deactivate
// Send zero velocities before releasing hardware interfaces.
// ═══════════════════════════════════════════════════════════════════════════
